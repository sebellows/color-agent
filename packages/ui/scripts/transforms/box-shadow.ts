...
/**
 * Parses a CSS box-shadow string into an object representation.
 * Supports multiple shadows separated by commas.
 *
 * Taken from the `react-native-unistyles` package.
 * @src {@link https://github.com/jpudysz/react-native-unistyles}
 *
 * @param str - The box-shadow string to parse.
 * @returns An array of BoxShadow objects or an empty array if no valid shadows are found.
 */
...
const parseBoxShadow = (str: string): BoxShadow | undefined => {
    ...
    if (![-1, 0, lastIndex].includes(insetIndex)) return undefined

    // if there is no inset, color can only be at the start or end
    const maybeColorsIndexes =
        insetIndex === -1 ? [0, lastIndex]
        : insetIndex === lastIndex ? [0, lastIndex - 1]
        : [1, lastIndex]
    ...
    if (!isValue(offsetX) || !isValue(offsetY)) return undefined

    const blurRadiusValue =
        isValue(blurRadius) ? Number.parseFloat(blurRadius as string) : undefined

    if (blurRadiusValue !== undefined && blurRadiusValue < 0) return undefined
