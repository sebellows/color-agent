...
import { emToPx } from './normalize-units'

const FONT_SIZE_REGEX = /text-(xs|sm|base|lg|xl|2xl|3xl|4xl|5xl|6xl|7xl|8xl|9xl)/
const LETTER_SPACING_REGEX = /(tracking-[a-z]+)/

export const letterSpacing = (utilities: Utilities, style: Style, classNames: string) => {
    const letterSpacingMatches = LETTER_SPACING_REGEX.exec(classNames)

    if (!letterSpacingMatches) return
